...
// --- TEXT LOOP (rotating loading messages) ---
type TextLoopProps = {
...
const DefaultLogo = () => (
  <div className="bg-primary text-primary-foreground rounded-md p-1.5">
    <svg width="16" height="16" viewBox="0 0 256 256" aria-hidden className="h-4 w-4">
      <path
        fill="currentColor"
...
type SuccessKind = "welcome" | "confirm" | "reset";
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "Flow", onBack }: AuthComponentProps) => {
  const { t } = useLocale();
  ...
  const [successKind, setSuccessKind] = useState<SuccessKind>("welcome");
  ...
  const fireConfetti = useCallback(() => {
    ...
    if (!fire) return;
    const defaults = { startVelocity: 30, spread: 360, ticks: 60, zIndex: 100, particleCount: 50 };
    fire({ ...defaults, origin: { x: 0, y: 1 }, angle: 60 });
    fire({ ...defaults, origin: { x: 1, y: 1 }, angle: 120 });
  }, []);

  useEffect(() => {
    if (authStep === "password") setTimeout(() => passwordInputRef.current?.focus(), 400);
    else if (authStep === "confirmPassword") setTimeout(() => confirmPasswordInputRef.current?.focus(), 400);
  }, [authStep]);
  ...
  const handleSignIn = async () => {
    if (!supabase) return showError("Sign-in isn’t configured yet.");
    if (!captchaOk) return;
  ...
  const handleSignUp = async () => {
    if (password !== confirmPassword) return showError(t.passwordsNoMatch);
    if (!supabase) return showError("Sign-up isn’t configured yet.");
    if (!captchaOk) return;
    setModalStatus("loading");
  ...
  const handleReset = async () => {
    if (!supabase) return;
    if (!isEmailValid) return showError(t.enterEmailForReset);
  ...
  const handleGoogle = async () => {
    if (!supabase) return showError("Google sign-in isn’t configured yet.");
  ...
  // Single "advance" entry point shared by Enter and the arrow buttons.
  const advance = () => {
    ...
    if (authStep === "confirmPassword" && isConfirmPasswordValid) void handleSignUp();
  };
  ...
  // Final step = the one whose arrow actually submits (needs captcha).
  const onFinalStep = (authStep === "password" && mode === "signIn") || authStep === "confirmPassword";
  const showCaptcha = captchaRequired && onFinalStep;

  const loadingMessages =
    mode === "signUp" ? [t.authCreatingAccount, t.authFinalizing] : [t.authSigningIn, t.authFinalizing];

  const successContent =
    successKind === "welcome"
      ? { icon: <PartyPopper className="w-12 h-12 text-green-500" />, text: t.welcomeAboard }
      : successKind === "confirm"
      ? { icon: <Mail className="w-12 h-12 text-primary" />, text: t.checkEmailBody(email.trim()) }
      : { icon: <Mail className="w-12 h-12 text-primary" />, text: t.resetPasswordSent };
    ...
    <div className="bg-background min-h-screen w-screen flex flex-col">
        ...
        {modalStatus !== "closed" && (
          ...
          >
            <motion.div
              ...
              className="relative bg-card border-2 border-border rounded-2xl p-8 w-full max-w-sm flex flex-col items-center gap-4 mx-2 text-center shadow-2xl"
            >
              ...
              {modalStatus === "error" && (
                <>
                  ...
                  <p className="text-lg font-medium text-foreground">{modalMessage}</p>
              ...
              {modalStatus === "loading" && (
                <TextLoop interval={1.6} stopOnEnd>
              ...
              {modalStatus === "success" && (
                <div className="flex flex-col items-center gap-4">
                  {successContent.icon}
                  <p className="text-lg font-medium text-foreground">{successContent.text}</p>
                </div>
              )}
            </motion.div>
          </motion.div>
      ...
      {onBack && (
        <button
          type="button"
          onClick={onBack}
          className="fixed top-4 left-4 z-20 inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-2 text-sm text-muted-foreground backdrop-blur-sm transition-colors hover:text-foreground"
        >
      ...
      {/* Brand */}
      <div className={cn("fixed top-4 z-20 flex items-center gap-2", "left-1/2 -translate-x-1/2")}>
        {logo}
        <h1 className="text-base font-bold text-foreground">{brandName}</h1>
      </div>

      <div className={cn("flex w-full flex-1 items-center justify-center bg-card", "relative overflow-hidden")}>
        ...
        >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {stepSubtitle && <p className="text-sm font-medium text-muted-foreground">{stepSubtitle}</p>}
            </motion.div>
          </AnimatePresence>

          {/* Google + OR (email step only) */}
          {authStep === "email" && (
            <BlurFade delay={0.1} className="w-full flex flex-col items-center gap-4">
              <p className="text-sm font-medium text-muted-foreground">{t.continueWith}</p>
              <GlassButton
                type="button"
              ...
              <div className="flex items-center w-full gap-2 py-1">
                ...
                <span className="text-xs font-semibold text-muted-foreground">{t.orDivider}</span>
                <hr className="w-full border-border" />
              </div>
            </BlurFade>
          )}
          ...
          <form onSubmit={(e) => e.preventDefault()} className="w-full space-y-6">
            ...
            <div className="relative w-full">
              <AnimatePresence>
                {authStep !== "email" && (
                  ...
                  >
                    <label className="text-xs text-muted-foreground font-semibold">{t.email}</label>
                  </motion.div>
                )}
              </AnimatePresence>
              <div className="glass-input-wrap w-full">
                <div className="glass-input">
                  ...
                  <input
                    ...
                    className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-muted-foreground focus:outline-none disabled:opacity-70"
                  />
                  {authStep === "email" && (
                    <div
                      className={cn(
                    ...
                    >
                      <GlassButton
                        ...
                        size="icon"
                        aria-label="Continue"
                        contentClassName="text-muted-foreground hover:text-foreground"
                      >
                  ...
                  <div className="relative w-full">
                    <AnimatePresence>
                      {password.length > 0 && (
                        <motion.div
                          ...
                          className="absolute -top-6 left-4 z-10"
                        >
                          <label className="text-xs text-muted-foreground font-semibold">{t.password}</label>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    <div className="glass-input-wrap w-full">
                      <div className="glass-input">
                        ...
                        <input
                          ...
                          onKeyDown={handleKeyDown}
                          className="relative z-10 h-full w-0 flex-grow bg-transparent text-foreground placeholder:text-muted-foreground focus:outline-none"
                        />
                        <div
                          className={cn(
                            "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out",
                            isPasswordValid && (mode === "signUp" || captchaOk) ? "w-10 pr-1" : "w-0"
                          )}
                        >
                          <GlassButton
                            type="button"
                    ...
                    {mode === "signIn" && (
                      <button
                        type="button"
                        onClick={handleReset}
                        className="mt-3 ml-4 text-sm text-muted-foreground hover:text-foreground transition-colors"
                      >
                        {t.resetPassword}
                      </button>
                    )}
                    ...
                    <div className="glass-input-wrap w-full">
                      <div className="glass-input">
                        ...
                        <div
                          className={cn(
                            "relative z-10 flex-shrink-0 overflow-hidden transition-all duration-300 ease-in-out",
                            isConfirmPasswordValid && captchaOk ? "w-10 pr-1" : "w-0"
                          )}
                        >
                          <GlassButton
                            type="button"
            ...
            {/* Captcha on the submitting step */}
            {showCaptcha && (
              <div className="flex justify-center">
                <TurnstileWidget onToken={setCaptchaToken} className="flex max-w-full justify-center overflow-x-auto" />
              </div>
            )}
          ...
          {authStep === "email" && (
            <p className="text-center text-sm text-muted-foreground">
              ...
              <button type="button" onClick={toggleMode} className="font-medium text-foreground hover:text-primary">
          ...
          {/* Legal */}
          {authStep === "email" && (
            <p className="text-center text-xs leading-relaxed text-muted-foreground max-w-[280px]">
              {t.signInLegalNote}{" "}
